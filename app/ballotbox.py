...
async def submit():
    ...
    # TODO if correct submit, else, go back
    try:
...
async def cast():
    try:
        ...
        logout_user()
        return await render_template("cast.html", stage="checkout", verification_code=verification_code, time=time)
...
async def spoil():
    try:
        ...
        time = datetime.fromtimestamp(receipt["timestamp"]).ctime()
        logout_user()
        return await render_template("spoil.html", stage="checkout", verification_code=verification_code, time=time)
    except Exception as e:
        app.log_exception(e)
        abort(400)
